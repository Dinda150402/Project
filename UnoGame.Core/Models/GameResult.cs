...
public sealed class GameResult<T> : GameResult
{
    ...
    private GameResult(bool success, T value, string? errorMessage)
    ...
    public static new GameResult<T> Fail(string errorMessage) =>
        new GameResult<T>(false, default!, errorMessage);
}
